"""
===============================================================================
Data Loading & Preprocessing Module (Crypto & Macro Assets)
===============================================================================

Description:
    This module handles the ingestion, alignment, and cleaning of multi-asset 
    financial data. It is designed to synchronize 24/7 cryptocurrency markets 
    (BTC, ETH) with traditional market hours (Stocks, Forex, Gold).

    Key functionalities:
    1. Data Loading: Reads CSVs for BTC, ETH, EUR, GLD, SPY, VIXY.
    2. Time Alignment:
       - Aligns all assets to a 24/7 Bitcoin minute-level timeline.
       - Handles timezone conversions (UTC to US/Eastern).
    3. Market Hours Logic:
       - Implements "Smart Fill" to handle gaps in traditional markets (04:00-12:00 EST).
       - Flags US trading hours (09:30-16:00 EST, Mon-Fri) for filtering.
    4. Preprocessing:
       - Interpolates missing values for Forex/Crypto pairs.
       - Generates boolean masks to indicate valid data points for each asset.

Usage:
    >>> from data_loader import build_merged_dataset, preprocess_merged_df
    >>> df = build_merged_dataset()
    >>> df_clean = preprocess_merged_df(df)
