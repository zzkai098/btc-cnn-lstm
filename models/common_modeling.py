...
Description:
    This module provides a centralized collection of helper functions for 
    assessing machine learning model performance. It includes standardized 
    plotting tools for time-series forecasting and loss curves, configured 
    with academic styling (serif fonts, minimalist grids) suitable for 
    publication or technical reports.

    Key functionalities:
    1. Visualization: 
       - `plot_true_pred`: Compares actual vs. forecasted values with 
         publication-ready styling.
       - `plot_loss`: Visualizes training and validation loss trajectories.
    2. Metrics Calculation:
       - `regression_metrics`: Computes standard regression stats (MSE, MAE, 
         RMSE, R²) combined with directional accuracy (Win Rate).
       - `win_rate`: Calculates the percentage of correct directional predictions.
    3. Global Styling:
       - Defines `STYLE_CONFIG` to enforce a consistent color palette and 
         font style across all figures.
...
Usage:
    >>> from utils_plot import plot_true_pred, regression_metrics
    >>> metrics = regression_metrics(y_test, y_pred)
    >>> plot_true_pred(y_test, y_pred, title="Model A Results")
